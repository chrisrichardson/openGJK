...
# Loop through all dof coordinates, first finding all possible collisions with
# the cells boundingboxtree, then using openGJK to compute the distance between
# the point and the cell.
for xi in x:
    possible_cells = dolfinx.geometry.compute_collisions_point(bbtree, xi)[0]
    actual_cells = []
    ...
    actual_cells_old = dolfinx.geometry.compute_entity_collisions_mesh(bbtree,
                                                                       mesh,
                                                                       xi)[0]
    # Compare results
